...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, attn_mask=None):
        ...
        q = self.w_q(q).view(batch_size, -1, self.n_heads, self.dim_head).transpose(1, 2)
        k = self.w_k(k).view(batch_size, -1, self.n_heads, self.dim_head).transpose(1, 2)
        v = self.w_v(v).view(batch_size, -1, self.n_heads, self.dim_head).transpose(1, 2)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, attn_mask):
        output = self.attention(q=x, k=x, v=x, attn_mask=attn_mask)
